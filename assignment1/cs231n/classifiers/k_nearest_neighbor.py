...
class KNearestNeighbor:
  ...
  def compute_distances_two_loops(self, X):
    ...
    for i in xrange(num_test):
      for j in xrange(num_train):
        ...
        #####################################################################
        # X[i] # The ith test example
        # self.X_train[j] # the jth training example
  ...
  def predict_labels(self, dists, k=1):
    ...
    for i in xrange(num_test):
      ...
      #########################################################################
      nearest_indices = np.argsort(dists[i])[:k] # the vector corresponding to test example i
      ...
      d = defaultdict(int)
      for close_y in closest_y:
        d[close_y] += 1

      max_class = max(d, key=d.get)
    
      # Check for duplicates 
      truly_nearest_classes = {k:v for k, v in d.items() if v == d[max_class]}

      y_pred[i] = sorted(truly_nearest_classes)[0]
      #########################################################################
      #                           END OF YOUR CODE                            # 
      #########################################################################
